-- Backup strategy: Keep working version of critical functions
-- Before modifying get_measurements_summary, create a backup

-- Create backup function (copy of current working version)
CREATE OR REPLACE FUNCTION get_measurements_summary_backup(p_user_id UUID)
RETURNS TABLE (
  metric TEXT,
  display_name TEXT,
  category TEXT,
...
GRANT EXECUTE ON FUNCTION get_measurements_summary_backup(UUID) TO authenticated;

-- Track this migration
INSERT INTO schema_version (version, description) 
VALUES ('20250108_function_backup', 'Created backup of get_measurements_summary function')
ON CONFLICT (version) DO NOTHING;

-- ROLLBACK INSTRUCTIONS:
-- If get_measurements_summary breaks, restore from backup:
